...
public class CompositeStatementObject extends AbstractStatement {
	...
	public List<TryStatementObject> getSWITCH_CASEStatement() {
		List<TryStatementObject> switchCaseStatements = new ArrayList<TryStatementObject>();
		if(this.getType().equals(StatementType.SWITCH_CASE))
			switchCaseStatements.add((TryStatementObject)this);
		for(AbstractStatement statement : statementList) {
			if(statement instanceof CompositeStatementObject) {
				CompositeStatementObject composite = (CompositeStatementObject)statement;
				switchCaseStatements.addAll(composite.getTryStatements());
			}
		}
		return switchCaseStatements;
	}
	
	public List<TryStatementObject> getEnhancedForStatement() {
		List<TryStatementObject> enhancedForStatements = new ArrayList<TryStatementObject>();
		if(this.getType().equals(StatementType.ENHANCED_FOR))
			enhancedForStatements.add((TryStatementObject)this);
		for(AbstractStatement statement : statementList) {
			if(statement instanceof CompositeStatementObject) {
				CompositeStatementObject composite = (CompositeStatementObject)statement;
				enhancedForStatements.addAll(composite.getTryStatements());
			}
		}
		return enhancedForStatements;
	}
	
	public List<TryStatementObject> getForStatement() {
		List<TryStatementObject> forStatements = new ArrayList<TryStatementObject>();
		if(this.getType().equals(StatementType.FOR))
			forStatements.add((TryStatementObject)this);
		for(AbstractStatement statement : statementList) {
			if(statement instanceof CompositeStatementObject) {
				CompositeStatementObject composite = (CompositeStatementObject)statement;
				forStatements.addAll(composite.getTryStatements());
			}
		}
		return forStatements;
	}
	
	public List<TryStatementObject> getDoStatements() {
		List<TryStatementObject> doStatements = new ArrayList<TryStatementObject>();
		if(this.getType().equals(StatementType.DO))
			doStatements.add((TryStatementObject)this);
		for(AbstractStatement statement : statementList) {
			if(statement instanceof CompositeStatementObject) {
				CompositeStatementObject composite = (CompositeStatementObject)statement;
				doStatements.addAll(composite.getTryStatements());
			}
		}
		return doStatements;
	}
	
	public List<TryStatementObject> getTernaryStatements() {
		List<TryStatementObject> ternaryStatements = new ArrayList<TryStatementObject>();
		if(this.getType().equals(StatementType.TERNARY))
			ternaryStatements.add((TryStatementObject)this);
		for(AbstractStatement statement : statementList) {
			if(statement instanceof CompositeStatementObject) {
				CompositeStatementObject composite = (CompositeStatementObject)statement;
				ternaryStatements.addAll(composite.getTryStatements());
			}
		}
		return ternaryStatements;
	}
	
	public List<TryStatementObject> getSuperConstructorInvocationStatements() {
		List<TryStatementObject> ternaryStatements = new ArrayList<TryStatementObject>();
		if(this.getType().equals(StatementType.SUPER_CONSTRUCTOR_INVOCATION))
			ternaryStatements.add((TryStatementObject)this);
		for(AbstractStatement statement : statementList) {
			if(statement instanceof CompositeStatementObject) {
				CompositeStatementObject composite = (CompositeStatementObject)statement;
